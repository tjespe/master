...
df_test["True_Return"] = y_test
df_test["Pred_Mean"]   = y_mean_test
df_test["Pred_Std"]    = y_std_test
...
plt.plot(df_test.index, df_test["True_Return"], label="Actual", color="black")
plt.plot(df_test.index, df_test["Pred_Mean"], label="Predicted Mean", color="red")
plt.fill_between(
    df_test.index,
    df_test["Pred_LB_95"],
    df_test["Pred_UB_95"],
...
plt.fill_between(
    df_test.index,
    df_test["Pred_Mean"] - df_test["Pred_Std"],
    df_test["Pred_Mean"] + df_test["Pred_Std"],
    color="blue",
    alpha=0.2,
    label="1 Std Dev"
...
plt.fill_between(
    df_test.index,
    df_test["Pred_Mean"] - 2 * df_test["Pred_Std"],
    df_test["Pred_Mean"] + 2 * df_test["Pred_Std"],
    color="green",
    alpha=0.2,
    label="2 Std Dev"
...
plt.figure(figsize=(12,6))
plt.plot(df_test.index[-100:], df_test["True_Return"].iloc[-100:], label="Actual", color="black")
plt.plot(df_test.index[-100:], df_test["Pred_Mean"].iloc[-100:], label="Predicted Mean", color="red")
plt.fill_between(
    df_test.index[-100:],
    df_test["Pred_LB_95"].iloc[-100:],
    df_test["Pred_UB_95"].iloc[-100:],
    color="red",
    alpha=0.2,
    label="95% Interval"
)
plt.fill_between(
    df_test.index[-100:],
    df_test["Pred_Mean"].iloc[-100:] - df_test["Pred_Std"].iloc[-100:],
    df_test["Pred_Mean"].iloc[-100:] + df_test["Pred_Std"].iloc[-100:],
    color="blue",
    alpha=0.2,
    label="1 Std Dev"
...
plt.fill_between(
    df_test.index[-100:],
    df_test["Pred_Mean"].iloc[-100:] - 2 * df_test["Pred_Std"].iloc[-100:],
    df_test["Pred_Mean"].iloc[-100:] + 2 * df_test["Pred_Std"].iloc[-100:],
    color="green",
    alpha=0.2,
    label="2 Std Dev"
...
#   df_test: DataFrame with index = test_dates, columns including "True_Return"
...
for subplot_i in range(10):
    ...
    # The actual return
    actual_return = df_test["True_Return"].iloc[idx]
