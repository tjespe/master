...
# %%
# Test for autocorrelation (with p-values for each lag)
from statsmodels.tsa.stattools import acf

for symbol in np.unique(data.train.tickers):
    s = data.train.sets[symbol]
    acf_vals = acf(s.y.squeeze(), nlags=20, qstat=True)
    p_vals = acf_vals[2]
    if any(p_vals < 0.05):
        print(f"Symbol {symbol} has significant autocorrelation at some lags.")
        print(p_vals)
    else:
        print(f"Symbol {symbol} has no significant autocorrelation at any lag.")
    plt.plot(acf_vals[0][1:])
    plt.title(f"ACF for {symbol}")
    for i, p in enumerate(p_vals):
        if i == 0:
            continue
        plt.text(i, acf_vals[0][i], f"{p:.2f}")
    plt.show()

# %%
# Create linear regression model to predict next day's return based on past returns
...
for symbol in np.unique(data.train.tickers):
    ...
    y = s.y[1:]
    lr.fit(X, y)
    print(f"Symbol {symbol}: R^2 = {lr.score(X, y)}")
    rmse = np.sqrt(np.mean((lr.predict(X) - y) ** 2))
    print(f"RMSE = {rmse}")
    rmse_naive = np.sqrt(np.mean((X - y) ** 2))
    print(f"RMSE naive = {rmse_naive}")
    print(f"Improvement: {100 * (rmse_naive - rmse) / rmse_naive:.2f}%")
    rmse_naive_mean = np.sqrt(np.mean((X - X.mean()) ** 2))
    print(f"RMSE naive mean = {rmse_naive_mean}")
    results.append(
        {
            "Symbol": symbol,
            "R^2": lr.score(X, y),
            ...
            "Improvement": 100 * (rmse_naive - rmse) / rmse_naive,
            "RMSE_naive_mean": rmse_naive_mean,
            "Improvement_mean": 100 * (rmse_naive_mean - rmse) / rmse_naive_mean,
...
# %%
# Do the same but with more lags
# Create a linear regression model
lr = LinearRegression()
...
for symbol in np.unique(data.train.tickers):
    s = data.train.sets[symbol]
    X = np.column_stack([s.y[i : -LOOKBACK_DAYS + i] for i in range(LOOKBACK_DAYS)])
    y = s.y[LOOKBACK_DAYS:]
    lr.fit(X, y)
    print(f"Symbol {symbol}: R^2 = {lr.score(X, y)}")
    rmse = np.sqrt(np.mean((lr.predict(X) - y) ** 2))
    print(f"RMSE = {rmse}")
    rmse_naive = np.sqrt(np.mean((0 - y) ** 2))
    print(f"RMSE naive = {rmse_naive}")
    print(f"Improvement: {100 * (rmse_naive - rmse) / rmse_naive:.2f}%")
    rmse_naive_mean = np.sqrt(np.mean((y - y.mean()) ** 2))
    print(f"RMSE naive mean = {rmse_naive_mean}")
    results.append(
        {
            "Symbol": symbol,
