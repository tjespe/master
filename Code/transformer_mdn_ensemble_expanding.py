...
if __name__ == "__main__":
    ...
    for i, (ticker, date) in enumerate(examples):
        ...
        ax.set_title(
            f"{date.strftime('%Y-%m-%d')} - Transformer-MDN-{paper_name} predicted return distribution for {ticker}",
            pad=15,
        )
    ...
    for ticker in example_tickers:
        ...
        ax.set_title(
            f"Evolution of Transformer-MDN-{paper_name} Mixture Weights for {ticker}"
        )
        ax.set_xlabel("Time")
