# File implementing a HARQ quantile regression model as a benchmark
# %%
# Import libraries
import pandas as pd
import numpy as np
...
capire_df["RQ"] = capire_df["RQ_5"] / (100.0**4)
capire_df
# %%
...
def create_harq_features(group):
    g = group.copy()
    g["RV_lag1"] = g["RV"].shift(1)
    g["RV_lag5"] = g["RV_lag1"].rolling(window=5).mean()
    g["RV_lag22"] = g["RV_lag1"].rolling(window=22).mean()
    g["RQ_lag1"] = g["RQ"].shift(1)
    # exactly the interaction term in Bollerslev et al. (2016)
    g["RV1_sqrtRQ"] = g["RV_lag1"] * np.sqrt(g["RQ_lag1"])
    return g
...
def forecast_symbol(symbol):
    ...
    for i in range(len(validation_data_symbol)):
        ...
        X = sample_data[["RV_lag1", "RV1_sqrtRQ", "RV_lag5", "RV_lag22"]]
        X = sm.add_constant(X)
        y = sample_data["LogReturn"]
        X_pred = symbol_data[["RV_lag1", "RV1_sqrtRQ", "RV_lag5", "RV_lag22"]].iloc[
            end : end + 1
        ]
...
# %%
# save the dataframe
results_df.to_csv(f"predictions/HARQ_qreg_{TEST_SET}.csv", index=False)
