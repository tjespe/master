...
vae.fit(X_train, epochs=5, batch_size=32, validation_data=(X_val, X_val), verbose=1)
...
for i in range(10):
    ...
    y_vals.append(y_val)
Z_train = np.vstack(Z_trains)
y_train_broadcasted = np.hstack(y_trains)
Z_val = np.vstack(Z_vals)
...
def build_latent_lstm_predictor(latent_dim):
    ...
    x = LSTM(32, activation="tanh")(inputs)  # single-step LSTM
    x = Dropout(0.1)(x)
...
Z_val_reshaped = Z_val.reshape(-1, 1, latent_dim)

latent_lstm.fit(
    Z_train_reshaped,
    y_train_broadcasted,
    validation_data=(Z_val_reshaped, y_val_broadcasted),
    epochs=10, # 10
    batch_size=32,
    verbose=1,
)
...
def predict_distribution(encoder, predictor, X, n_samples=1000):
    ...
    for i in range(len(X)):
        ...
        sys.stdout.write(
            f"\r"+"#"*int((i/len(X))*terminal_width)+f" [{i+1}/{len(X)}]"
        )
        ...
        print("z_samples_reshaped shape:", z_samples_reshaped.shape)
        print(z_samples_reshaped)

        # Predict
        raw_preds = predictor.predict(z_samples_reshaped, verbose=0)
...
# Let's do it for X_val
n_samples = 200
test_asset_data = validation_data.sets[TEST_ASSET]
y_dist_test = predict_distribution(encoder, latent_lstm, test_asset_data.X, n_samples=n_samples)
...
df_validation = pd.DataFrame(index=test_asset_data.y_dates)

df_validation["True_Return"] = test_asset_data.y
...
def compute_confidence_intervals(y_samples, confidence_levels):
    ...
    intervals = np.zeros((B, num_levels, 2))
...
def compute_expected_shortfall(y_samples, VaR_estimates):
    """
    Compute Expected Shortfall (ES) for VAE + LSTM.
    
    Arguments:
    - y_samples: Sampled predictions from LSTM (shape: (batch, n_samples))
    - var_estimates: Value-at-risk estimates for each sample (shape: (batch,))
    
    Returns:
    - ES values (shape: (batch,))
    """
    B = y_samples.shape[0]  # Batch size
    es_values = np.zeros(B)

    for i in range(B):
        # Sort the sample predictions
        s = np.sort(y_samples[i])

        # Compute the expected shortfall
        es_values[i] = np.mean(s[s <= VaR_estimates[i]])

    return es_values  # Returns array of shape (batch,)

# Compute NLL for the test set
df_validation["NLL"] = compute_nll(test_asset_data.y, y_dist_test) 
df_validation["CRPS"] = compute_crps(test_asset_data.y, y_dist_test)
...
for i, cl in enumerate(confidence_levels):
    ...
    es_alpha = 1-(1-cl)/2
    df_validation[f"ES_{format_cl(es_alpha)}"] = compute_expected_shortfall(y_dist_test, intervals[:, i, 0])
