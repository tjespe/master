...
# %%
# Imports -- assume these are mostly the same environment as the MDN example
import numpy as np
import pandas as pd
...
def build_generator(
    lookback_days, num_features, noise_dim, hidden_units=16, dropout=0.2
):
    """
...
def build_discriminator(lookback_days, num_features, hidden_units=16, dropout=0.2):
    """
...
generator.save(f"models/{MODEL_NAME}_generator.keras")
discriminator.save(f"models/{MODEL_NAME}_discriminator.keras")
...
samples_test = cgan_predict_distribution(generator, X_test, T=T_SAMPLES)
